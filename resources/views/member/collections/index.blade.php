...
@section('content')
  ...
  <div class="mt-8 px-4 flex justify-between">
    <h1 class="text-gray-600 montserrat-semibold text-2xl">Koleksi Saya</h1>
  </div>
    ...
    <div class="grid grid-cols-8 gap-4 mt-2">
      @foreach ($collections as $item)
        <div class="col-span-8 sm:col-span-4 md:col-span-4 lg:col-span-2 xl:col-span-2">
          <a href="/m1/books/{{ $item->book->id }}/show">
            <div class="bg-white shadow-lg rounded-lg px-4 py-6 mx-4 my-4">
              <div class="overflow-hidden mx-auto h-64 bg-gray-200 rounded-md">
                <img src="{{ asset('storage/'.$item->book->book_cover) }}" alt="">
              </div>
              <h3 class="mt-8 montserrat-semibold text-gray-600">{{ $item->book->title }}</h3>
              <p class="mt-2">{{ $item->book->author.' | '.$item->book->publisher }}</p>
  ...
  @else
    ...
    <p class="text-center mt-6">There is no book collections!</p>
  @endif

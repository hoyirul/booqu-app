    ...
    <div class="col-md-12">
      <div class="card mb-4">
        ...
        <div class="card-body demo-vertical-spacing demo-only-element">
          ...
          <form action="/v1/memberships" method="POST">
            @csrf
                  ...
                  <div class="input-group">
                    ...
                    <select name="role_id" id="role_id" class="form-control @error('role_id') is-invalid @enderror">
                      @foreach ($roles as $row)
                        <option value="{{ $row->id }}" {{ (old('role_id') == $row->id) ? 'selected' : '' }}>{{ $row->role_name }}</option>
                      @endforeach
                    </select>
                  ...
                  <div class="input-group">
                    ...
                    <select name="membership_id" id="membership_id" class="form-control @error('membership_id') is-invalid @enderror">
                      @foreach ($memberships as $row)
                        <option value="{{ $row->id }}" {{ (old('membership_id') == $row->id) ? 'selected' : '' }}>{{ $row->package }}</option>
                      @endforeach
                    </select>
